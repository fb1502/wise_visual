...
profile_id = config.get('id','profile_id')
account_id = config.get('id','account_id')

private_key_path = './private.pem'  # Change to private key path
base_url = 'https://api.transferwise.com'

if profile_id == '' or account_id == '':
...
def get_statement(
    ...
    signature="", 
    currency = 'EUR', 
    interval_start = (datetime.now(timezone.utc) - timedelta(days=10)).isoformat(), 
